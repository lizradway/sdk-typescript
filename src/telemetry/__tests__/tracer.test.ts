...
describe('Tracer', () => {
  ...
  describe('initialization', () => {
    ...
    it('should accept trace attributes', () => {
      const traceAttributes = {
        'custom.key': 'custom.value',
      }
      const tracerWithConfig = new Tracer(traceAttributes)
  ...
  describe('span creation', () => {
    ...
    it('should create an agent loop cycle span', () => {
      const messages: Message[] = [
        new Message({
          role: 'user',
          content: [new TextBlock('Hello')],
  ...
  describe('Property 1: Span Lifecycle Completeness', () => {
    ...
    it('should start and end agent loop cycle spans with success status', () => {
      const messages: Message[] = [
        new Message({
          role: 'user',
          content: [new TextBlock('Test')],
  ...
  describe('Property 2: Attribute Consistency (Tool Call Tracing)', () => {
    ...
    it('should maintain attribute consistency across multiple tool call spans', () => {
      ...
      if (span1) {
        const toolResult1 = new ToolResultBlock({
          toolUseId: 'a-1',
          status: 'success',
      ...
      if (span2) {
        const toolResult2 = new ToolResultBlock({
          toolUseId: 'b-1',
          status: 'success',
  ...
  describe('Property 7: Span Parent-Child Relationships', () => {
    it('should maintain parent-child relationship for agent loop cycle spans', () => {
      const messages: Message[] = [
        new Message({
          role: 'user',
          content: [new TextBlock('Test')],
      ...
      if (parentSpan) {
        ...
        if (cycleSpan) {
          tracer.endAgentLoopSpan(cycleSpan)
        }
    ...
    it('should maintain parent-child relationship with custom attributes', () => {
      ...
      if (parentSpan) {
        ...
        if (childSpan) {
          tracer.endAgentLoopSpan(childSpan)
        }
  ...
  describe('Property 5: Content Block Mapping Round-Trip', () => {
    ...
    it('should map ToolUseBlock to OTEL format and preserve tool information', () => {
      const messages: Message[] = [
        new Message({
          role: 'assistant',
          content: [new TextBlock('I will use a tool')],
        }),
      ]
    ...
    it('should handle mixed content blocks in a single message', () => {
      const messages: Message[] = [
        new Message({
          role: 'user',
          content: [new TextBlock('First part'), new TextBlock('Second part')],
        }),
      ]
