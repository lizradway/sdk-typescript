...
export class Tracer {
  ...
  startModelInvokeSpan(options: StartModelInvokeSpanOptions): Span | null {
    ...
    const modelTrace = new LocalTrace('stream_messages', this._local.currentCycle)
    this._local.currentModel = modelTrace
  ...
  startToolCallSpan(options: StartToolCallSpanOptions): Span | null {
    ...
    const toolTrace = new LocalTrace(`Tool: ${tool.name}`, this._local.currentCycle)
  ...
  startAgentLoopSpan(options: StartAgentLoopSpanOptions): Span | null {
    ...
    const cycleNumber = this._local.traces.length + 1
    const cycleTrace = new LocalTrace(`Cycle ${cycleNumber}`)
    cycleTrace.rawName = cycleId
